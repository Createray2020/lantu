  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-8 py-8">
      <div className="text-center mb-6">
        <div className="text-brand text-xs tracking-[0.3em] mb-1">STEP 2 · 完善你的規劃</div>
        <h1 className="font-serif text-3xl">補上基本資料與財務現況</h1>
        <p className="text-tx2 text-sm mt-2">填完現況就能看到你的缺口與願景達成率，接著可以選一位教練陪你。</p>
      </div>
      ...
      <section className="rounded-2xl bg-panel2 border border-line p-5 sm:p-6 mb-4 shadow-e1">
        ...
        <div className="mt-5 flex items-center gap-3">
          <button onClick={onSubmitCurrent} disabled={saveStatus === "saving"}
            className="font-bold text-onbrand bg-brand hover:bg-brand2 disabled:opacity-60 px-6 py-2.5 rounded-lg">
            {saveStatus === "saving" ? "計算中…" : "送出，看我的缺口與達成率"}
          </button>
          {saveStatus === "error" && <span className="text-danger text-sm">⚠ {saveErr}</span>}
        </div>
      </section>

      {/* 缺口 ＋ 願景達成率 */}
      {showGap && crossEmpty && (
        <section className="rounded-2xl bg-gradient-to-br from-panel to-panel2 border border-brand/30 p-6 mb-4">
          <h2 className="font-serif text-lg mb-2">📊 你的缺口與願景達成率</h2>
          {/* 四格全 0 就不要端一個 0% 出來：那不是「你差很多」，是「還沒有資料」。 */}
          <p className="text-sm text-tx">先填上你的收入與支出，我們才算得出缺口。</p>
        </section>
      )}
      ...
      {showGap && !crossEmpty && (
        <section className="rounded-2xl bg-gradient-to-br from-panel to-panel2 border border-brand/30 p-6 mb-4">
          ...
          <div className="grid sm:grid-cols-2 gap-6 items-center">
            <div>
              <div className="text-tx2 text-sm mb-1">願景達成率</div>
              <div className="font-serif text-4xl text-brand2 mb-2">{ratePct}%</div>
              <div className="h-3 rounded-full bg-tx/8 overflow-hidden">
                <div className="h-full rounded-full" style={{ width: `${ratePct}%`, background: ratePct >= 100 ? "var(--ok)" : "var(--brand)" }} />
              </div>
            </div>
            <div className="text-sm text-tx space-y-1.5">
              ...
              <div className="flex justify-between text-danger"><span>每月缺口</span><b>NT$ {ntfmt(gap.monthlyGap)}</b></div>
              <div className="flex justify-between border-t border-line pt-1.5"><span>目前淨資產</span><b>{wan(gap.netWorth)} 萬</b></div>
            </div>
          </div>
          {/* ⚠️ 缺口是規劃的起點，不是成績單。
      ...
      {/* 選擇教練 */}
      <section className="rounded-2xl bg-panel2 border border-line p-5 sm:p-6 mb-16 shadow-e1">
        <h2 className="font-serif text-lg mb-1">🤝 選擇你的教練</h2>
        {localLink.state === "linked" ? (
          <div className="mt-3">
            <p className="text-ok text-sm">✓ 你已連結教練{localLink.coachName ? `：${localLink.coachName}` : ""}。你們現在可以一起規劃了。</p>
            <button onClick={onRevoke} disabled={reqStatus === "sending"}
              className="mt-3 text-danger hover:text-danger text-sm border border-danger/30 rounded-lg px-4 py-2 disabled:opacity-50">
              解除與教練的連結
            </button>
            {reqStatus === "error" && <span className="ml-3 text-danger text-sm">⚠ {reqErr}</span>}
          </div>
        ...
        <div className="mt-6 text-center">
          <Link href="/portal" className="text-sm text-tx2 hover:text-tx underline underline-offset-4">先回客戶首頁</Link>
        </div>
      </section>
    </div>
  );
