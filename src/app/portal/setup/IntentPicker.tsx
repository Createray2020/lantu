...
export default function IntentPicker({ value, onChange }: { value: Intent; onChange: (next: Intent) => void }) {
  ...
  const toggleTarget = async (t: string) => {
    const i = must.indexOf(t);
    if (i < 0) return set({ mustHave: [...must, t] });
    if (t === DEFAULT_TARGET && !await confirmDialog("退休幾乎是必然會發生的。\n取消後，我們不會幫你算退休金需求。\n\n確定取消嗎？")) return;
  ...
  return (
    <section className="rounded-2xl bg-panel2 border border-line p-5 sm:p-6 mb-4 shadow-e1">
      ...
      <p className="text-11 text-tx3 mb-4">選起來的，我們就當作一定要達成；沒選的不會再問你相關問題。</p>

      {/* 關注議題 */}
      <div className="text-xs text-tx2 mb-2">關注議題 · 可複選</div>
      <div className="flex flex-wrap gap-2 mb-6">
        {PURPOSES.map((p) => {
          const on = value.purposes.includes(p);
          return (
            <button key={p} type="button" onClick={() => togglePurpose(p)}
              className={`rounded-full px-3 py-1.5 text-xs font-bold border ${on ? "bg-brand text-onbrand border-brand" : "bg-panel3 text-tx2 border-line hover:border-brand2"}`}>
              {on ? "✓ " : ""}{p}
            </button>
          );
        })}
      ...
      <div className="flex items-center flex-wrap gap-2 mb-2">
        <span className="text-xs text-tx2">人生目標 · 選了＝一定要達成</span>
        <span className="flex-1" />
        <button type="button" onClick={selectAll}
          className="text-xs font-bold text-brand2 border border-brand/40 rounded-lg px-3 py-1 hover:bg-tx/5">
          我全都要 · 跑完整人生模擬
        </button>
      </div>
        ...
        {metas.map((m) => {
          ...
          return (
            <button key={m.name} type="button" onClick={() => toggleTarget(m.name)}
              ...
              {on && (
                <span className="absolute -top-2 -right-2 grid place-items-center w-[22px] h-[22px] rounded-full bg-onbrand text-brand2 border border-brand font-serif text-xs font-extrabold">
                  {i + 1}
                </span>
              )}
              <span className="block font-bold text-13 leading-tight">{m.name}</span>
              <span className={`block text-11 mt-0.5 ${on ? "text-onbrand/65" : "text-tx3"}`}>
      ...
      {/* 優先序 */}
      <div className="text-xs text-tx2 mb-1">哪個最重要？</div>
      <p className="text-11 text-tx3 mb-2">拖曳或用 ◀ ▶ 排出順序。錢不夠時，我們會從最後一項開始調整。</p>
      {must.length === 0 ? (
        <div className="text-xs text-tx3 py-1">
        ...
        <div className="flex flex-col gap-2" onDragOver={(e) => e.preventDefault()}>
          {must.map((t, i) => (
            <div key={t} draggable
              ...
              <span className="font-serif text-sm font-extrabold text-brand2 min-w-[14px] text-center">{i + 1}</span>
              <span className="flex-1 font-bold text-13 text-tx">{t}</span>
              <button type="button" aria-label="往前" onClick={() => move(i, -1)}
                className="w-10 h-10 rounded-md border border-line text-tx2 text-13 hover:border-brand2 hover:text-brand2">◀</button>
