...
export default function SurveyForm({
  ...
  return (
    <div className="space-y-6">
      ...
      {done.length > 0 && (
        <section>
          <h2 className="text-sm font-bold text-tx2 mb-2">已完成</h2>
          <div className="space-y-2">
            {done.map((c) => (
              <div key={c.id} className="rounded-xl border border-line bg-panel px-4 py-3 shadow-e1">
                <div className="flex flex-wrap items-baseline gap-2">
                  <span className="font-semibold">{c.moduleName || "教練服務"}</span>
                  <span className="text-xs text-tx3">{c.signedAt ?? ""}</span>
                  <span className="text-xs text-ok">已於 {c.surveyAt} 回覆 ✓</span>
                </div>
                {c.answers?.some(Boolean) && (
                  <ul className="mt-2 space-y-1 text-xs text-tx2">
                    {questions.map((q, i) => (
                      c.answers?.[i] ? <li key={i}><b className="text-tx">{q}</b>：{c.answers[i]}</li> : null
                    ))}
                  </ul>
                )}
              </div>
...
function OneSurvey({
  ...
  return (
    <section className="rounded-2xl border border-line bg-panel p-5 shadow-e1">
      <div className="flex flex-wrap items-baseline gap-2 mb-1">
        <h2 className="font-semibold text-base">{c.moduleName || "教練服務"}</h2>
        {c.signedAt && <span className="text-xs text-tx3">{c.signedAt}</span>}
      </div>
      <p className="text-xs text-tx2 mb-4">
      ...
      <div className="space-y-4">
        {questions.map((q, i) => (
          <label key={i} className="block">
            <span className="block text-sm text-tx mb-1">{i + 1}. {q}</span>
            <textarea
              rows={3}
              value={answers[i] ?? ""}
              disabled={pending}
              onChange={(e) => setAnswers((a) => a.map((x, k) => (k === i ? e.target.value : x)))}
              className="w-full rounded-lg bg-canvas border border-line2 px-3 py-2 text-sm text-tx outline-none focus:border-brand leading-relaxed"
            />
          </label>
        ))}
      </div>
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          ...
          className="rounded-lg bg-brand text-onbrand font-bold px-4 py-2 text-sm disabled:opacity-40"
        >
          {pending ? "送出中…" : "送出回饋"}
        </button>
        {msg && (
