...
import { countPendingIntroductions } from "@/lib/coachApplyStore";

export const dynamic = "force-dynamic";

// 教練端頁首徽章用：待接受的客戶連結申請數 ＋ 待回覆的共同執案邀請數 ＋ 待確認的報聘申請數。
// 三種都在 /dashboard/requests 這一頁處理，紅點也就只有一顆。
export async function GET() {
  try {
    const coach = await ensureCoach();
    if (!coach || coach.status !== "active") return NextResponse.json({ count: 0 });
    const [links, collab, intros] = await Promise.all([
      countPendingForCoach(coach.id),
      countPendingInvitesForCoach(coach.id),
      countPendingIntroductions(coach.id),
    ]);
    return NextResponse.json({ count: links + collab + intros });
  } catch {
