...
import { loadParams, saveRanks, saveSettings, saveThresholds } from "@/lib/comp/repo";
import {
  clearAllAction, loadV4Action, saveRanksAction, saveSettingsAction, saveThresholdsAction,
} from "./actions";
...
beforeEach(() => {
  ...
  asMock(loadParams).mockResolvedValue({ settings: {}, ranks: [], thresholds: [] });
});
...
describe("loadV4Action", () => {
  ...
  it("已自訂的數值不被覆蓋", async () => {
    asMock(loadParams).mockResolvedValue({ settings: { payoutDay: 20 }, ranks: [], thresholds: [] });
