    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <div className="rounded-xl border border-line bg-panel p-4 space-y-3 shadow-e1">
          <h3 className="text-sm font-bold border-l-[3px] border-brand2 pl-2">案件條件</h3>
          {modules.length > 0 && (
            <label className="flex items-start gap-2 text-sm">
              <span className="w-24 text-tx2 pt-1">服務模塊</span>
          ...
          <label className="flex items-center gap-2 text-sm">
            <span className="w-24 text-tx2">顧問費</span>
          ...
          <label className="flex items-center gap-2 text-sm">
            <span className="w-24 text-tx2">執案者職級</span>
          ...
          {!selfBoth && !companyLead && (
            <label className="flex items-center gap-2 text-sm">
              <span className="w-24 text-tx2">推廣者職級</span>
        ...
        <div className="rounded-xl border border-line bg-panel p-4 shadow-e1">
          <h3 className="text-sm font-bold border-l-[3px] border-brand2 pl-2 mb-2">
            輔導鏈（由下而上，不含本人）
          </h3>
          ...
          <div className="space-y-2">
            {chain.map((c, i) => (
              <div key={i} className="flex items-center gap-2 text-sm">
                <span className="text-xs text-tx2 w-14">第 {i + 1} 層</span>
                ...
                <button type="button" className="text-danger text-xs px-1"
      ...
      <div className="rounded-xl border border-line bg-panel p-4 shadow-e1">
        <div className="flex items-center gap-3 mb-3">
          <h3 className="text-sm font-bold border-l-[3px] border-brand2 pl-2">逐層分潤明細</h3>
          <div className="flex-1" />
          <span className={`text-sm ${res.balanced ? "text-ok" : "text-danger"}`}>
        ...
        {res.warnings.length > 0 && (
          <p className="mt-2 text-xs text-brand2">尚未設定：{res.warnings.join("；")}</p>
        )}
      </div>

      {/* 快速情境 */}
      <div className="rounded-xl border border-line bg-panel p-4 shadow-e1">
        <h3 className="text-sm font-bold border-l-[3px] border-brand2 pl-2 mb-1">辦法範例</h3>
