  ...
  return (
    <div className="min-h-screen bg-canvas text-tx">
      <header className="sticky top-0 z-30 backdrop-blur bg-canvas/85 border-b border-line">
        <div className="max-w-5xl mx-auto px-5 sm:px-8 h-16 flex items-center justify-between">
          ...
          <a href={LINE_URL} target="_blank" rel="noopener noreferrer"
             className="text-sm font-bold text-onbrand bg-brand hover:bg-brand2 px-4 py-2 rounded-lg">
            預約 1 對 1 說明
          </a>
        </div>
      </header>
      ...
      <section className="border-y border-line bg-field">
        <div className="max-w-5xl mx-auto px-5 sm:px-8 py-16">
          <div className="text-center mb-10">
            <div className="text-brand text-xs tracking-[0.3em] mb-2">你會拿到什麼</div>
            <h2 className="font-serif text-2xl sm:text-3xl mb-2">一套已經在跑的規劃系統</h2>
            <p className="text-tx2 text-sm">不是簡報上的示意圖。下面每一項都在線上運作中，你可以自己去按。</p>
          </div>
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              ...
              <div key={t} className="rounded-xl bg-panel border border-line p-5 shadow-e1">
                <div className="font-serif text-[17px] mb-2 text-brand2">{t}</div>
                <p className="text-tx2 text-sm leading-relaxed">{d}</p>
              </div>
            ))}
          </div>
          <div className="text-center mt-8">
            <Link href="/passport" className="inline-block text-sm font-bold text-onbrand bg-brand hover:bg-brand2 px-6 py-3 rounded-lg">
              直接去玩玩看客戶端的試算 →
            </Link>
          </div>
        </div>
      ...
      <section className="max-w-3xl mx-auto px-5 sm:px-8 py-16">
        ...
        <p className="text-tx2 text-sm mb-6">與其讓你來了才發現不對，不如現在就講清楚。以下任何一條你看了會皺眉，這裡大概不適合你。</p>
      ...
      <section className="border-t border-line bg-field">
        <div className="max-w-5xl mx-auto px-5 sm:px-8 py-16">
          <div className="text-center mb-9">
            ...
            <p className="text-tx2 text-sm">起點不同，前面幾個月要走的路不一樣。</p>
          </div>
      ...
      <section className="max-w-5xl mx-auto px-5 sm:px-8 py-16">
        <div className="text-center mb-10">
          <div className="text-brand text-xs tracking-[0.3em] mb-2">接下來會發生什麼</div>
      ...
      <section className="border-t border-line bg-field">
        <div className="max-w-3xl mx-auto px-5 sm:px-8 py-16 text-center">
          ...
          <div className="mt-8 pt-6 border-t border-line">
            <p className="text-tx2 text-sm mb-3">已經談過了，或想直接提出申請？</p>
            <Link href="/apply"
      ...
      <footer className="border-t border-line">
        <div className="max-w-5xl mx-auto px-5 sm:px-8 py-8 flex flex-col sm:flex-row items-center justify-between gap-3 text-sm text-tx3">
          <span className="font-serif tracking-[0.14em]">嵐途 LAN TU</span>
          <div className="flex items-center gap-4">
            <Link href="/login" className="hover:text-tx2">已經是嵐途教練？登入</Link>
            <Link href="/home" className="hover:text-tx2">← 回官網首頁</Link>
          </div>
        </div>
      </footer>
    </div>
