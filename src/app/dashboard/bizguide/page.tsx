...
function Card({ id, title, sub, children }: { id?: string; title: string; sub?: string; children: React.ReactNode }) {
  return (
    <section id={id} className="rounded-xl border border-line bg-panel p-5 scroll-mt-20 shadow-e1">
      <h2 className="text-sm font-bold border-l-[3px] border-brand2 pl-2 mb-1">{title}</h2>
      {sub && <p className="text-xs text-tx3 mb-3">{sub}</p>}
      <div className="text-sm text-tx leading-relaxed space-y-3">{children}</div>
    </section>
  );
}
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm border-collapse min-w-[520px]">
        <thead>
          <tr>{head.map((h) => <th key={h} className="px-3 py-2 font-semibold text-xs text-tx2 text-left whitespace-nowrap">{h}</th>)}</tr>
        </thead>
        <tbody>
          {rows.map((r, i) => (
            <tr key={i}>{r.map((cell, j) => (
              <td key={j} className={`px-3 py-2 border-t border-line align-top ${j === 0 ? "font-semibold text-tx whitespace-nowrap" : ""}`}>{cell}</td>
            ))}</tr>
          ))}
        </tbody>
      </table>
...
export default async function BizGuidePage() {
  ...
  return (
    <main className="min-h-dvh bg-canvas text-tx">
      <DashboardHeader {...hp} />
      <section className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-4">
        <div>
          ...
          <p className="text-xs text-tx3 mt-1">
            教練的背景知識與現場工具。資料基準 {BIZ_TAX_BASIS}，法規數字統一維護於 <code className="text-brand2">src/lib/bizTax.ts</code>。
            本頁不構成稅務或法律建議。
          </p>
        </div>
        ...
        <nav className="flex flex-wrap gap-2">
          {NAV.map(([id, label]) => (
            <a key={id} href={`#${id}`} className="text-xs px-3 py-1.5 rounded-full border border-line text-tx2 hover:border-brand2 hover:text-brand2">
              {label}
            </a>
          ))}
        </nav>
        ...
        <Card title="核心命題" sub="所有提案的總綱">
          ...
          <p>
            ...
            <b className="text-tx">收入與資產同源（沒有分散效果）</b>、公私財務交纏、
            現金流由他自己決定、退休等於事業退場。
          </p>
        </Card>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <ol className="list-decimal list-inside space-y-1 text-13" start={16}>
                ...
                <li className="text-tx font-semibold">你把多少比例的身家放在這家公司？這個比例你安心嗎？</li>
              </ol>
            </div>
          </div>
        </Card>
        ...
        <Card id="vat" title="401 與營業稅 40 系列" sub="401 只是一個減法：銷項稅額 − 進項稅額 = 應納或溢付">
          ...
          />
          <p>
            <b className="text-tx">40 系列速記</b>：沒有 402 這個編號。401 是一般稅額計算（加值型）、403 是特種稅額計算、
            404 是總繳單位彙總、405/406 是外國事業或跨境電商相關；401／403／404 兼具申報書與繳款書功能。
          </p>
          <p className="text-xs text-tx3">
            401 與損益表對不起來是正常的（開票時點差異、零稅率、非營業收入、預收款、視為銷貨…）。
            <b className="text-tx">對不起來不是罪，說不出理由才是</b>——建議每年結帳時做一張「401 與帳載收入差異調節表」，被函查時可以直接交出去。
          </p>
        </Card>
        ...
        <Card id="red" title="紅線：節稅、避稅、逃稅" sub="最重要的一列是「事實基礎」">
          ...
          <p>
            ...
            <b className="text-tx">逃稅的問題在「事實」——這件事根本沒發生，沒有辯論空間。</b>
          </p>
          <Table
            head={["層級", "依據", "代價"]}
            rows={[
        ...
        <Card id="car" title="租賃車：被誤解最深的一個做法" sub="這不是「買或租」的技術問題，是「這台車是不是真的營業用」的事實問題">
          ...
          />
          <p>
            財政部 111 年 1 月令釋：承租自用乘人小客車，有下列任一情形<b className="text-tx">核屬分期付款買賣性質，進項稅額不得扣抵</b>——
        ...
        <Card id="legal" title="合法的替代路徑" sub="只講風險不給出路，是沒有用的">
          <ol className="list-decimal list-inside space-y-2">
            <li><b className="text-tx">憑證管理制度化</b>（解決 80% 的問題）：能開發票的供應商優先——表面上貴 5%，但那 5% 可以扣抵；小額支出有法定的替代憑證方式；臨時工可納保、可辦扣繳，外包可簽約並申報執行業務所得。<b className="text-tx">付了錢就一定有合法的認列方式，差別只在流程有沒有做。</b>當下就分類，不要年底回頭補——回頭補的過程本身就是虛列的溫床。</li>
            <li><b className="text-tx">申報方式的年度試算</b>：書審 vs 查帳，每年五月前重新試算，不要沿用去年。</li>
            <li><b className="text-tx">合法的費用結構設計</b>：薪資、董監酬勞、租金的合理配置。前提是要有契約、要辦扣繳、要有合理性。</li>
            <li><b className="text-tx">租稅優惠盤點</b>：產創條例研發投資抵減、智慧機械／5G／資安設備投資抵減、中小企業增僱員工薪資加成減除、未分配盈餘實質投資減除（產創 §23-3）、產業專法優惠。這些都是稅法明文給的，用了不會有任何風險。</li>
            <li><b className="text-tx">資本支出的時點規劃</b>：固定資產進項稅額可計入得退稅限額，是少數能把留抵轉為現金退稅的合法途徑。</li>
            <li><b className="text-tx">主動補正（稅捐稽徵法 §48-1）</b>：自動補報並補繳所漏稅款者，凡屬未經檢舉、未經調查之案件，處罰一律免除，涉及刑事責任者並得免除其刑（補繳稅款須加計利息）。<b className="text-brand2">這是一扇會關上的門</b>——一旦被檢舉或已進入調查程序就不能用了。</li>
          </ol>
        </Card>
        ...
        <p className="text-xs text-tx3 pb-6">
