...
export default function CollabInviteList({ invites }: { invites: Invite[] }) {
  ...
  if (!list.length) return <p className="text-tx2">目前沒有待回覆的共同執案邀請。</p>;

  return (
    <div className="space-y-3">
      {err && <div className="text-danger text-sm">⚠ {err}</div>}
      {list.map((iv) => (
        <div key={iv.id} className="rounded-xl bg-panel2 border border-info/25 p-4 flex items-center justify-between gap-4 shadow-e1">
