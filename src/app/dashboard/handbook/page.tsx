...
export default async function HandbookPage() {
  ...
  const th = "px-3 py-2 font-semibold text-xs text-tx2 text-left";
  ...
  return (
    <main className="flex-1 bg-canvas text-tx min-h-screen">
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-4">
        <div>
          <h1 className="text-xl font-bold">業務制度說明</h1>
          <p className="text-sm text-tx2 mt-1">
            目前生效版本 <b className="text-brand2">{version.version}</b>
        ...
        {/* 服務模塊 */}
        <div className={card}>
          <h2 className="text-sm font-bold border-l-[3px] border-brand2 pl-2 mb-2">服務模塊與分潤架構</h2>
        ...
        {TABS.filter((t) => ["maintain", "case", "payout"].includes(t.id)).map((tab) => (
          <div key={tab.id} className={card}>
            <h2 className="text-sm font-bold border-l-[3px] border-brand2 pl-2 mb-2">
              {tab.label}
              <span className="ml-2 text-xs font-normal text-tx3">辦法{tab.law}</span>
            </h2>
            {tab.sections.map((sec) => (
              <div key={sec.title} className="mb-3 last:mb-0">
                <div className="text-xs text-tx2 mb-1">{sec.title}</div>
                <dl className="grid gap-x-4 gap-y-1 sm:grid-cols-2">
                  {sec.fields.map((f) => (
                    <div key={f.key as string} className="flex justify-between gap-3 text-sm border-b border-line py-1">
                      <dt className="text-tx">{f.label}</dt>
                      <dd className={
                        s[f.key] === undefined ? "text-tx3" : "text-brand2 font-semibold"
                      }>
                        {show(s[f.key], f.type)}
                      </dd>
                    </div>
        ...
        {/* 問卷 */}
        <div className={card}>
          <h2 className="text-sm font-bold border-l-[3px] border-brand2 pl-2 mb-2">回饋問卷（結案要件）</h2>
          <ol className="list-decimal list-inside text-sm text-tx space-y-1">
