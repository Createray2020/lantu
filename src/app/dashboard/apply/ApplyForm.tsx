  ...
  return (
    <div className="w-full max-w-xl">
      <div className="text-center">
        ...
        <p className="text-tx2 text-sm leading-relaxed mb-2">
          送出後由嵐途審核（含費用確認），開通即可使用完整的教練工作台。
        </p>
        <p className="text-tx3 text-xs mb-7">登入帳號：{email || "（未提供 email）"}</p>
      </div>
          ...
          return (
            <button
              ...
              className={`text-left rounded-lg border p-3 transition ${
                on ? "border-brand bg-brand/10" : "border-line2 hover:border-line2"
              }`}
            >
        ...
        {d.licenses.map((r, i) => (
          <div key={i} className="grid sm:grid-cols-[1fr_auto] gap-2 items-start border border-line rounded-md p-2">
            ...
            <button
              ...
              className="text-tx2 hover:text-tx text-xs px-2 py-1"
            >
              移除
            </button>
          </div>
      ...
      {d.licenses.length < LICENSE_MAX && (
        <button
          ...
          className="text-xs text-brand hover:text-brand2 mb-6"
        >
          ＋ 新增一項證照
        </button>
      )}
      ...
      <button
        ...
        disabled={busy || !ready}
        className="w-full font-bold text-onbrand bg-brand hover:bg-brand2 px-6 py-2.5 rounded-lg disabled:opacity-40 disabled:cursor-not-allowed"
      >
      ...
      <div className="mt-8 pt-6 border-t border-line text-center">
        <p className="text-tx3 text-xs mb-3">不是要當教練？</p>
        <Link href="/portal" className="text-sm text-tx2 hover:text-tx underline underline-offset-4">
          我是客戶，前往我的財務規劃 →
        </Link>
      </div>
    </div>
