    ...
    <div className="space-y-4">
      <section className="rounded-2xl bg-panel2 border border-line p-5 sm:p-6 shadow-e1">
        <div className="flex items-baseline justify-between flex-wrap gap-2 mb-4">
          ...
          <span className="text-[11px] text-tx3">已回答 {g.answered} / {GATE_Q.length}　·　約兩分鐘</span>
        </div>

        <ol className="space-y-3">
          {GATE_Q.map((q, i) => (
            <li key={q} className="flex items-start gap-3 border-t border-line pt-3 first:border-0 first:pt-0">
              <span className="font-serif text-[13px] text-tx3 w-5 shrink-0 pt-1.5">{i + 1}</span>
              <div className="flex-1 min-w-0">
                <p className="text-[13.5px] leading-relaxed">
                  {q}
                  {i === GATE_VETO_INDEX && (
                    <span className="ml-2 text-[10.5px] text-brand2 border border-brand/50 rounded px-1.5 py-0.5 whitespace-nowrap">
                      有時效性
                    </span>
                  )}
                </p>
                {ans[i] === "否" && (
                  <p className="text-[12px] text-tx2 mt-1.5 leading-relaxed">
                    → {GATE_GUIDE[i].mean}
                  </p>
                )}
              </div>
      ...
      <section className={`rounded-2xl border p-5 sm:p-6 ${st.ring} ${st.bg}`}>
        <div className="flex items-baseline gap-3 flex-wrap">
          ...
          {done && <span className="text-[12px] text-tx2">勾「否」{g.no} 題</span>}
        </div>
        <p className="text-[13.5px] text-tx mt-2 leading-relaxed">{lamp.note}</p>

        {!done && (
          <p className="text-[12px] text-tx3 mt-3">
        ...
        {done && g.no > 0 && (
          <>
            <h3 className="text-[12px] text-tx2 mt-5 mb-2">
              建議優先處理（最多 {MAX_GAPS_AT_ONCE} 項）
            </h3>
            <ol className="space-y-2.5">
              {gaps.map((x, i) => (
                <li key={x.q} className="rounded-lg bg-field/70 border border-line p-3">
                  <p className="text-[13px] font-bold text-tx">{i + 1}. {x.mean}</p>
                  <p className="text-[12px] text-tx2 mt-1 leading-relaxed">{x.next}</p>
                </li>
              ))}
            </ol>
        ...
        {done && g.no === 0 && (
          <p className="text-[13px] text-tx mt-4">
        ...
        <section className="rounded-2xl bg-panel2 border border-line p-5 sm:p-6 shadow-e1">
          ...
          {status === "saved" ? (
            <div className="rounded-lg bg-ok-solid/20 border border-ok-solid p-4">
              <p className="text-[13.5px] text-ok font-bold">已存進你的規劃</p>
              <p className="text-[12.5px] text-tx mt-1 leading-relaxed">
                企業財務規劃已經在你的規劃裡打開了——之後接上教練時，他會直接看到公司概況、公私勾稽與這份檢核。
              </p>
              <Link href="/portal" className="inline-block mt-3 rounded-lg bg-brand text-onbrand font-bold text-[13.5px] px-4 py-2.5 hover:bg-brand2">
                去看我的規劃 →
              </Link>
            </div>
          ) : (
            <>
              {err && (
                <p className="text-[12.5px] text-danger mb-3 leading-relaxed">
                  {err}
                  {needPassport && (
              ...
              <div className="flex flex-wrap gap-2">
                {signedIn ? (
                  <button
                    ...
                    onClick={save}
                    disabled={pending}
                    className="rounded-lg bg-brand text-onbrand font-bold text-[13.5px] px-4 py-2.5 hover:bg-brand2 disabled:opacity-50"
                  >
                    {pending ? "存檔中…" : "存進我的規劃"}
                  </button>
                ) : (
                  ...
                  <Link
                    ...
                    className="rounded-lg bg-brand text-onbrand font-bold text-[13.5px] px-4 py-2.5 hover:bg-brand2"
                  >
                    註冊並存下這份檢核
                  </Link>
                )}
                <Link href="/coaches" className="rounded-lg border border-line2 text-tx2 font-bold text-[13.5px] px-4 py-2.5 hover:border-brand2 hover:text-brand2">
                  找一位教練談談
                </Link>
              </div>
            </>
