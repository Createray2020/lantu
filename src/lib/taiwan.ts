...
/**
 * 投保金額分級表（115.01.01 生效）全 58 級。來源：衛福部中央健保署「投保金額分級表」。
 * ⚠️ 這是給教練「對得到級距」用的——健保跟勞保是兩張表，企業主自付 100%，
 *    級距選錯的代價是勞保那邊的 3.3 倍。逐年調整時只改這裡（與 lantu-app.html 的鏡像）。
 */
export const NHI_SALARY_TABLE: number[] = [
